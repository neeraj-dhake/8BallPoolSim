...
Vector3D AABB::GetIntersection_Ray_Plane(Vector3D _ray, Vector3D _plane) const{
	float cos_theta = Vector3D::DotProduct(_ray, _plane) / Vector3D::Magnitude(_plane);
	return _ray * Vector3D::Magnitude(_plane) / cos_theta;
}

float AABB::nearest_distance(Vector3D _ray) const{
	...
	for (int i = 0; i < 6; i++) {
		...
		if (Vector3D::DotProduct(current, _ray) < 0) {
			continue;
		}
		else {
			if (current.lies_between(center - dimensions / 2, center + dimensions / 2))
				break;
		}
	}
